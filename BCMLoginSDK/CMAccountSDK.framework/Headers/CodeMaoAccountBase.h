...
@end
@interface CodeMaoAccountBase: NSObject
//测试模式下优先级最高
...
/// 设置极验的 AppKey
- (void)setGeetestKey:(NSString *)Key;
- (CodeMaoAccountBase* _Nonnull (^)(NSString * _Nonnull))setGeetestKey;

/// 设置腾讯防水墙的 AppKey
- (void)setWaterRoofKey:(NSString *)Key;
- (CodeMaoAccountBase* _Nonnull (^)(NSString * _Nonnull))setWaterRoofKey;
...
/**
 ...
 @return 单例
 */
+ (CodeMaoAccountBase*)defaultManager;

/**
 设置 Debug 模式;
 */
- (void)setMode:(CMLoginSDKMode)Mode;
- (CodeMaoAccountBase* (^)(CMLoginSDKMode Model))setMode;
/**
 设置 Debug 模式 & 服务器地址
...
- (CodeMaoAccountBase* (^)(NSString * _Nonnull Key))setWeChatAppKey;
...
- (CodeMaoAccountBase* (^)(NSString * _Nonnull Key))setQQAppKey;
/**
 QQ登录接口
 //MAKR: 先设置 QQ AppKey
 @param CompletionHandler 返回结果
